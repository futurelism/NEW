...
class ClipModelConfig(BaseModelConfig):
    # 图像编码器参数
    image_encoder_type: str = "ViT"  # 可以是 'ViT' 或 'ResNet'
    ...
    vit_embedding_dim: int = 768

    # 如果是 ResNet
    resnet_depth: int = 50  # ResNet-50 默认值
    ...
    max_position_embeddings: int = 77  # CLIP默认文本长度
    use_gated_mlp: bool = False
    use_bias: bool = True
    use_qkv_bias: bool = True
    activation: ActivationType = ActivationType.GELU
    norm: NormType = NormType.LAYER_NORM
    post_attn_norm: bool = False
    vocab_size: int = 49408  # CLIP默认词表大小

    # CLIP 特有参数
    projection_dim: int = 512  # 两个模态共享的投影维度
    temperature: float = 0.07  # CLIP对比损失的温度系数

    @staticmethod
    def get_name():
        return "openai/clip-base"
@dataclass
class ClipViTB32ModelConfig(ClipModelConfig):
...
@dataclass
class ClipViTB16ModelConfig(ClipModelConfig):
    image_encoder_type: str = "ViT"
    image_resolution: int = 224
    image_patch_size: int = 16
    vit_num_layers: int = 12
...
@dataclass
class ClipViTL14ModelConfig(ClipModelConfig):
    image_encoder_type: str = "ViT"
    image_resolution: int = 224
    image_patch_size: int = 14
    vit_num_layers: int = 24
    vit_num_heads: int = 16
    vit_embedding_dim: int = 1024
